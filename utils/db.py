...
class DB:
    ...
    def create_db(self) -> None:
        ...
        self.cur.execute('''
            CREATE TABLE IF NOT EXISTS reaction4role(
            guild_id INTEGER,
    ...
    def update_guild_settings(self, guild_id, analyze=-1, log_id=-1, role_id=-1, distribution=-1, game_id=-1):
        ...
        if log_id == -1 and role_id == -1 and distribution == -1 and analyze == -1 and game_id == -1:
            self.cur.execute(
                'INSERT INTO guilds VALUES(?, 0, NULL, NULL, NULL, NULL, NULL) ON CONFLICT (id) DO NOTHING',
                (guild_id,)
            )

        if analyze != -1:
            self.cur.execute(
                'INSERT INTO guilds VALUES(?, ?, NULL, NULL, NULL, NULL, NULL) '
        ...
        if log_id != -1:
            self.cur.execute(
                'INSERT INTO guilds VALUES(?, 0, ?, NULL, NULL, NULL, NULL) '
        ...
        if distribution != -1:
            self.cur.execute(
                'INSERT INTO guilds VALUES(?, 0, NULL, NULL, ?, ?, NULL) '
    ...
    def update_reaction4role(self, guild_id, role_id, reaction):
        ...
        try:
            self.cur.execute(
                'INSERT INTO reaction4role VALUES(?, ?, ?)',
    ...
    def update_user(self, guild_id, user_id, user_name, len_message=0, num_attach=0, voting=(0, 0), comm=True):
        ...
        self.cur.execute(
            'INSERT INTO users VALUES(?, ?, ?, ?, 1, ?, ?, ?) ON CONFLICT DO UPDATE '
